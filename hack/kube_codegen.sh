...
CODEGEN_VERSION_SPEC="${KUBE_CODEGEN_TAG:-v0.33.0-beta.0}"
...
# Install from a local go module
function kube::codegen::internal::install() {
    local binaries=($@)
    (
        # To support running this from anywhere, first cd into this directory,
        # and then install with forced module mode on and fully qualified name.
        mkdir -p "${KUBE_CODEGEN_ROOT}/helpers"
        cd "${KUBE_CODEGEN_ROOT}/helpers"
        if [ ! -e go.mod ]; then
            echo module internal.codegen > go.mod
        fi
        # shellcheck disable=2046 # printf word-splitting is intentional
        go get "k8s.io/code-generator@${CODEGEN_VERSION_SPEC}"
        if [ ${#binaries[*]} -gt 0 ]; then
          echo installing binaries "${binaries[@]}"
          for p in $(printf "k8s.io/code-generator/cmd/%s " "${binaries[@]}"); do
              echo $p
              go install "${p}@${CODEGEN_VERSION_SPEC}"
          done
        fi
    )
}
...
function kube::codegen::gen_openapi() {
    ...
    kube::codegen::internal::install openapi-gen
...
function kube::codegen::gen_register() {
    ...
    kube::codegen::internal::install register-gen
